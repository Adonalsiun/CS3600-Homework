...
def getNNExtraData(fileString="datasets/poker-hand-training-true.data", limit=100000):
    """
    Returns a limited number of examples from the poker hand dataset.

    """
    # Use Framework from getNNPenData
    examples = []
    lineNum = 0
...
def buildExamplesFromExtraData(size=10000):
    """
    Build Neural-network friendly data structure from the poker hand data.
    """
    # Use framework for buildExamplesFromPenData
    if size != 10000:
        pokerDataTrainList = getNNExtraData("datasets/poker-hand-training-true.data", int(.8 * size))
        pokerDataTestList = getNNExtraData("datasets/poker-hand-testing.data", int(.2 * size))
    else:
        pokerDataTrainList = getNNExtraData("datasets/poker-hand-training-true.data")
        pokerDataTestList = getNNExtraData("datasets/poker-hand-testing.data")
