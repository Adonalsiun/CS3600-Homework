...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Implementation from Textbook, PG. 815, figure 13.13 for Elimination Ask

        # Initialize input variables, create counter
        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()
        newBeliefs = util.Counter()

        # For each position in all positions, get updated probability and insert into factors
        for ghostPosition in self.allPositions:
            ...
            newBeliefs[ghostPosition] = probability * self.beliefs[ghostPosition]
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Implementation from Textbook, PG. 811, figure 13.11 for Enumeration Ask

        # Create inputs for observed values.
        newBeliefs = DiscreteDistribution()

        # For each value of i, find the new position and old position
        for oldPosition in self.allPositions:
            ...
            # Find distribution over X, initially empty, and fill.
            for newPosition, probability in newPositionDistribution.items():
                newBeliefs[newPosition] += oldPositionProbability * probability
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        # Implementation from Textbook, PG. 915, figure 14.17 for Particle Filtering

        # Input for all legal positions.
        numPositions = len(self.legalPositions)
        # Loop from i to n for samples with new replacement.
        for i in range(self.numParticles):
            particles = self.legalPositions[i % numPositions]
    ...
    def elapseTime(self, gameState):
        ...
        for i, particle in enumerate(self.particles):
            if particle not in positionDistributions:
                newPosDist = self.getPositionDistribution(gameState, particle)
                positionDistributions[particle] = newPosDist
            self.particles[i] = positionDistributions[particle].sample()
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        # Generate permutations and shuffle them
        permutations = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(permutations)
        # Calculate number of copies of permutations needed, and remainder particles.
        numCopies = self.numParticles // len(permutations)
        remainder = self.numParticles % len(permutations)

        # Populate the particle list with copies and shuffle for uniform distribution
        self.particles = permutations * numCopies + permutations[:remainder]
